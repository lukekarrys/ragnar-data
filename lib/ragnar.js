...
const { deepPick, deepOmit, mapValuesNumeric, mapValuesOmitEmpty, defaultAt, mToFt, orderByParams } = require('./utils')
const { defaultPick, defaultOmit, defaultSort, defaultRound, ragnarUrl } = require('./constants')
...
module.exports = ({
  ...
  const fetchData = (url) => axios
    ...
    .then((data) => cache ? fs.outputJson(cachePath(url), data).then(_.identity(data)) : data)
